...
- **痛殴者武器**：新增 `pummeler` 近战物品（`+1` 攻击伤害、耐久 `388`），主手左键与副手右键均可对单体目标造成伤害；不继承剑/斧类，无横扫与破盾能力。
- **痛殴者暴击强化效果**：新增 `pummeler_crit_boost` MobEffect；仅当主手与副手均装备痛殴者时，连续暴击倍率按 `1.5 → 2.0 → 2.5 → 3.0` 递增（最高 `3.0`）；受到伤害、解除双持或 `5` 秒未成功命中后移除。

### 变更

- **版本号**：`gradle.properties` 中 `mod_version` 自 `0.1.4a` 调整为 `0.1.4b`。
- **创造模式标签页**：在工具类条目中加入痛殴者。
- **语言与模型 datagen**：补全痛殴者物品名、tooltip 及暴击强化效果的中英文翻译；`ModItemModelGen` 注册手持模型。
...
- **发布资源生成**：PR 合并发布流程在构建前执行 `runData`，确保自动发布的 JAR 包含由 datagen 生成的中英文语言文件与客户端资源，避免整合包环境中物品名退回显示翻译键。

### 开发者提示

- 发布流程仍以 `gradle.properties` 中的 `mod_version` 作为 Release 标签来源；修改语言 datagen 后无需手动提交 `assets/*/lang/*.json`，发布工作流会在构建前重新生成。
